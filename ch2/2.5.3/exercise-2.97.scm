	  ...
	  (else
	   (let ((g (gcd-generic n d)))
	     (cond ((not (and (pair? n) (pair? d)))
		   ...
		   (else
		    (let ((rs (apply-generic 'reduce n d)))
		      rs)))))))
  ;(trace make-rat)
  (define (add-rat x y)
...
(define (install-sparse-polynomial-package)
  ...
  (define (pseudoremainder-terms a b)
    (let ((int-factor (expt (coeff (first-term b))
  ...
  (define (gcd-polys a b)
    (cond ((not (same-variable? (variable a) (variable b)))
	  ...
	  ((> (order (first-term (term-list b)))
	      (order (first-term (term-list a))))
	   (let ((result (gcd-terms (term-list b)
					(term-list a))))
...
;    (trace neg-term)
    (define (neg p)
      (make-poly (variable p)
		 (map  (lambda (t)
			 (neg-term t))
		       (term-list p))))
    (define (get-biggest-integer a b)
      (cond ((> a b)
	     a)
	    (else b)))
    
    (define (reduce-terms n d g)

	    (let ((o2 (order (first-term g)))
		  (o1 (get-biggest-integer (order (first-term n))
					   (order (first-term d)))))

	      (let ((integerizing-factor (expt (coeff (first-term g))
					       (- (+ o1 1)
						  o2))))
		(let ((i-term
		       (make-term  0 integerizing-factor)))
		  (list (car (div-terms (mul-term-by-all-terms i-term  n) g))
			(car (div-terms (mul-term-by-all-terms i-term  d) g)))))))
    ...
    (put 'neg '(sparse-polynomial)
	 (lambda (p)
	   (tag (neg p))))
    (put 'reduce '(sparse-polynomial sparse-polynomial)
	 (lambda (n d)
	   (cons (tag (car (reduce-poly n d)))
		 (tag (cadr (reduce-poly n d))))))
    ...
    (put 'project '(sparse-polynomial)
	 (lambda (x)
	   (cond ((=zero? (order (first-term (term-list x))))
...
;  (trace convert-to-sparse)
  ...
  (put 'reduce '(polynomial polynomial)
       (lambda (p1 p2)
	 (reduce-poly (convert-to-sparse p1)
		      (convert-to-sparse p2))))
  ...
  (put 'project '(polynomial)
       (lambda (p)
	 (let ((rs (apply-generic 'project p)))
	    (cond ((pair? rs)
		   (tag rs))
		  (else rs))))) 
