(define (add-interval interval1 interval2)
  (make-interval (+ (lower-bound interval1) (lower-bound interval2))
		 (+ (upper-bound interval1) (upper-bound interval2))))

(define (mul-interval x y)
  (let ((ll (* (lower-bound x) (* lower-bound y)))
	(lu (* (lower-bound x) (* upper-bound y)))
	(ul (* (upper-bound x) (* lower-bound y)))
	(uu (* (upper-bound x) (* upper-bound y))))
    (make-interval (min ll lu ul uu)
		   (max ll lu ul uu))))
(define (>0? x)
  (> x 0))
; the possibilities of the different signs is 2^(2-1) * 2^(2-1)
(define (mul-interval-sign x y)
  (let ((xl (lower-bound x))
	(xu (upper-bound x))
	(yl (lower-bound y))
	(yu (upper-bound y)))
    (cond ((and (>0? xl)  ;(+ + + +)
		(>0? xu)
		(>0? yl)
		(>0? yu))
	   (make-interval (* xl yl) (* xu yu)))
	  ((and (>0? xl) ; (+ + - +)
		(>0? xu)
		(not (>0? yl))
		(>0? yu))
	   (make-interval (* xu yl) (* xu yu)))
	  ((and (>0? xl) ; (+ + - -)
		(>0? xu)
		(not (>0? yl))
		(not (>0? yu)))
	   (make-interval (* xu yl) (* xl yu)))
	  ((and (not (>0? xl)) ; (- + + +)
		(>0? xu)
		(>0? yl)
		(>0? yu))
	   (make-interval (* xl yu) (* xu yu)))
	  ((and (not (>0? xl)) ; (- + - +)
		(>0? xu)
		(not (>0? yl))
		(>0? yu))
	   (make-interval (* xu yl) (* xu yu)))
	   ((and (not (>0? xl)) ; (- + - -)
		(>0? xu)
		(not (>0? yl))
		(not (>0? yu)))
	   (make-interval  (* xu yl) (* xl yl)))
	  ((and (not (>0? xl)) ; (- - - -)
		(not (>0? xu))
		(not (>0? yl))
		(not (>0? yu)))
	   (make-interval (* xl yl) (* xu yu)))
	  ((and (not (>0? xl)) ; (- - - +)
	       (not (>0? xu))
	       (not (>0? yl))
	       (>0? yu))
	   (make-interval (* xl yu) (* xu yl)))
	   ((and (not (>0? xl))	; (- - - -)
		 (not (>0? xu))
		 (not (>0? yl))
		 (not (>0? yu)))
	    (make-interval (* xl yl) (* xu yu))))))
	     
		
	  


(define (div-interval x y)
  (cond ((= (width y) 0)
	 (error "can't divide by an interval of width 0")))
  (mul-inerval x
	       (make-interval (/ 1.0 (upper-bound y))
			      (/ 1.0 (lower-bound y)))))

(define (make-interval a b)
  (cons a b))

(define (upper-bound x)
  (max (car x) (cdr x)))

(define (lower-bound x)
  (min (car x) (cdr x)))

(define (sub-interval x y)
  (add-interval x
		(make-interval
		 (- (lower-bound y))
		 (- (upper-bound y)))))

(define (width x)
  (/ (- (upper-bound x) (lower-bound x)) 2))


(define (make-center-percent center tolerance)
  (let ((margin (* (/ tolerance 100.0) center)))
    (make-interval (+ center margin) (- center margin))))

(define (percent interval)
  (let ((margin (/ (- (upper-bound interval)
		      (lower-bound interval))
		   2.0)))
    (let ((center (- (upper-bound interval) margin)))
      (cond ((or (> center 0) (< center 0))
	     (* (/ margin (abs center)) 100.0))
	    (else (error "null center"))))))
